...
class Fusion(nn.Module):
    def __init__(self, input_size, _seq_len, _embedding_dim, num_layers, _hidden_dim, batch_size):
        ...
        self._conv2 = nn.Sequential(nn.Conv1d(in_channels=_embedding_dim, out_channels=16, kernel_size=(1, 2), stride=(1, 2)),
                                    nn.ReLU(),
                                    nn.MaxPool2d((1, 2))
                                    )
        ...
        self._classifier = nn.Sequential(nn.Linear(in_features=16 * 8 * 78 + _hidden_dim * math.floor(((seq_len-2)/2+1)/2), out_features=100),
                                         # nn.Tanh(),
                                         nn.Hardtanh(min_val=-0.5, max_val=0.5),
                                         nn.Dropout(),
    ...
    def forward(self, _audio, _lyric, hidden_state):
        ...
        _audio_flat = _audio.contiguous().view(-1, 16*8*78)
        ...
        _lyric = self._conv2(_lyric)
        _lyric = _lyric.squeeze(2).permute(0, 2, 1)
        _lyric = _lyric.contiguous().view(BATCH_SIZE, -1, 16)
        _lyric_lstm, hidden_state = self._lstm(_lyric, self.hidden_state)
        # print(_lyric_lstm.shape)
        _lyric_flat = _lyric_lstm.contiguous().view(_lyric_lstm.size(0), -1)


        x = torch.cat((_audio_flat, _lyric_flat), dim=1)
        score = self._classifier(x)
        return F.softmax(score, dim=1), hidden_state
...
for fold, (train_ids, val_ids) in enumerate(kfold.split(train_val_set)):
    ...
    # Saving the model
    save_path = f'model/model_f16-fold-{fold}.pth'
    save_opt = f'model/opt_f16_fold-{fold}.pth'
